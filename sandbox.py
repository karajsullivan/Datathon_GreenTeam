...
# Create a function to extract the html document from a given url
def getHTML(self, url):

    # Request HTML document of a given url
    response = requests.get(url)


# Get the URL of the discussions page
base_url = "https://live.paloaltonetworks.com/t5/discussions/ct-p/members"

# Read the content of the discussions page
base_page = requests.get(base_url)

# Parse through the discussions page
...
# Select elements at the top level of the page
# list(base_soup.children)

# Check the type of each element in the list
#[type(item) for item in list(base_soup.children)]

# Select the html tag and its children
#html = list(base_soup.children)[1]
base_soup.find_all("h3 a")
#
container = base_soup.find_all("h3 a")
container
...
generalPost_page = requests.get(generalPost_url)
bestPractice_page = requests.get(bestPractice_url)
cortexXDR_page = requests.get(cortexXDR_url)
cortextXSOAR_page = requests.get(cortexXSOAR_url)
customSignatures_page = requests.get(customSignatures_url)
endpoint_page = requests.get(endpoint_url)
globalProtect_page = requests.get(globalProtect_url)
nextGenFirewalls_page = requests.get(nextGenFirewalls_url)
prismaAccessInsights_page = requests.get(prismaAccessInsights_url)
panorama_page = requests.get(panorama_url)
prismaAccess_page = requests.get(prismaAccess_url)
virusTotal_page = requests.get(virusTotal_url)
prismaCloud_page = requests.get(prismaCloud_url)
threatVulnerability_page = requests.get(threatVulnerability_url)
vmPrivate_page = requests.get(vmPrivate_url)
vmPublic_page = requests.get(vmPublic_url)
